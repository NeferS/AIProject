...
public class BoardDataStructure {
    ...
    public BoardDataStructure(String s, int nr, int nc) {
        ...
        for (int i = 0; i < nr; ++i) {
            for (int j = 0; j < nc; ++j) {
                this.board[i][j] = s.charAt(k) == 'B' ? new Cell(Color.BLACK) : (s.charAt(k) == 'W' ? new Cell(Color.WHITE) : new Cell(null));
                ++k;
            }
        }
    }
    ...
    public void set(int i, int j, Color col) {
        this.board[i][j].set(col);
    }

    public void set(int[] t, Color col) {
        this.board[t[0]][t[1]].set(col);
    }

    public void add(int i, int j, Color col, int pieces) {
    ...
    public void add(int[] t, Color col, int pieces) {
        this.add(t[0], t[1], col, pieces);
    }

    public void remove(int i, int j, Color col, int pieces) {
        if (this.board[i][j].getColour() != null && !this.board[i][j].getColour().equals((Object)col)) {
    ...
    public void remove(int[] t, Color col, int pieces) {
